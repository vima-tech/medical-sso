...
四个容器不等于四份开销。实测本机栈稳态（压测 200 并发后）：

| 容器 | 内存 | 上限 | 说明 |
| --- | --- | --- | --- |
| Keycloak | 460 MB | 640 MB | 认证内核，占大头 |
| 统一身份管理平台 | 215 MB | 448 MB | Spring Boot |
| PostgreSQL | 31 MB | 256 MB | |
| 网关 | 5 MB | 64 MB | nginx，可忽略 |
| **合计** | **约 710 MB** | 1.4 GB | |

**容器数量不是内存占用的来源，JVM 的堆推算才是。** 两个 compose 都显式写了 `mem_limit` 和堆参数，原因是：不设 `mem_limit` 时 JVM 按**宿主机总内存**推算最大堆——在一台 31 GB 的机器上，Keycloak 的默认 `MaxRAMPercentage=70` 会把最大堆算成 20.4 GB，门户算成 7.29 GB，而两者实际堆用量各只有一百多 MB。多出来的 RSS 全是为这份用不上的额度准备的 GC 结构与元数据开销。加上限之后同一套栈从 1.51 GB 降到约 710 MB，功能与并发表现不变。

所以「为了省资源把容器合并成一个」是没有收益的：省下的只有 nginx 那几 MB，两个 JVM 该占多少还占多少。要省内存就调这两个值，不要动拓扑。

负载增长时同时上调 `mem_limit` 与对应的 `MaxRAMPercentage`，只调其中一个不会生效——堆上限是按容器上限的百分比算出来的。判断依据是容器内存使用率持续高于 80%，或日志出现 GC 频繁/OOM。
