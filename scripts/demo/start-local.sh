...
public_url="$(sso_public_url)"
issuer="${public_url}/auth/realms/medical"

# 演示子系统仍以 jar 跑在宿主机上，需要先构建。
# 统一身份管理平台不在这里构建——它由 compose 按 deploy/Containerfile 构建成镜像。
echo "构建接入组件与演示子系统..."
...
sso_compose_dev up -d --build
# Podman Compose 对同名标签不会自动替换既有容器；显式重建门户，避免出现
# “镜像已经更新，但 18081 仍运行旧镜像摘要”的假更新。
sso_compose_dev up -d --force-recreate --no-deps portal
...
SSO_ISSUER="$issuer" \
java -jar samples/boot3/target/medical-sso-demo-0.1.0.jar &
demo_pid=$!
SSO_ISSUER="$issuer" \
java -jar samples/boot2/target/medical-sso-demo-boot2-0.1.0.jar &
demo_boot2_pid=$!
# 已有账号体系的子系统：保留原登录，桥接统一身份，支持自助绑定
SSO_ISSUER="$issuer" \
java -jar samples/legacy/target/medical-sso-demo-legacy-0.1.0.jar &
demo_legacy_pid=$!
# 「改不动的系统」演示：一个对统一认证一无所知的上游，前面挂接入网关
python3 scripts/demo/upstream.py &
upstream_pid=$!
SSO_ISSUER="$issuer" \
java -jar integration/access-proxy/target/medical-sso-access-proxy-0.1.0.jar &
gateway_pid=$!
...
echo "按 Ctrl+C 停止演示子系统；平台本身用 ./scripts/sso.sh stop 停止。"
